...
import { ArrowLeft, ExternalLink, ShieldCheck, ArrowRight, CheckCircle2, Download, Layers, Cable, Wrench } from "lucide-react";
...
export async function generateStaticParams() {
  const locales = ["en", "he"];
  return TSUBAKI_PRODUCTS.flatMap((p) =>
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const product = TSUBAKI_PRODUCTS.find((p) => p.id === productId);
  if (!product) return {};
  return {
    title: `${product.name} — Tsubaki Kabelschlepp via RSTECH Electronics Israel`,
    description: product.description,
    alternates: {
      canonical: `https://www.rstech.co.il/${locale}/companies/tsubaki/products/${productId}`,
      languages: {
        en: `https://www.rstech.co.il/en/companies/tsubaki/products/${productId}`,
        he: `https://www.rstech.co.il/he/companies/tsubaki/products/${productId}`,
        "x-default": `https://www.rstech.co.il/en/companies/tsubaki/products/${productId}`,
      },
    },
  };
}
...
const CATEGORY_ICON: Record<string, React.ElementType> = {
  "cable-carriers": Layers,
  cables: Cable,
  systems: Wrench,
};

const CATEGORY_GRADIENT: Record<string, string> = {
  "cable-carriers": "from-orange-100 to-orange-50",
  cables: "from-amber-100 to-amber-50",
  systems: "from-slate-100 to-slate-50",
};

const CATEGORY_ACCENT: Record<string, string> = {
  "cable-carriers": "text-orange-400",
  cables: "text-amber-500",
  systems: "text-slate-400",
};
...
export default async function TsubakiProductPage({ params }: Props) {
  ...
  const Icon = CATEGORY_ICON[product.category] ?? Layers;
  const gradient = CATEGORY_GRADIENT[product.category] ?? CATEGORY_GRADIENT["cable-carriers"];
  const accent = CATEGORY_ACCENT[product.category] ?? CATEGORY_ACCENT["cable-carriers"];
                    ...
                    <StaggerGrid className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {related.map((r) => (
                        <StaggerCard key={r.id}>
                          <Link
                            href={`/companies/tsubaki/products/${r.id}`}
                          ...
                          >
                            <h3 className="font-semibold text-[#1a202c] text-sm mb-1 group-hover:text-orange-600 transition-colors">
                              {r.name}
                            </h3>
                ...
                <div className="bg-[#0F172A] rounded-2xl p-6 border border-slate-700">
                  <div className="space-y-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-slate-300">Tsubaki Kabelschlepp GmbH</div>
                    </div>
                    <div>
              ...
              <FadeUp delay={0.1}>
                <div className="space-y-2">
                  <Link
                    ...
                    className="flex items-center gap-2 text-sm text-[#4a5568] hover:text-orange-600 transition-colors cursor-pointer"
                  >
                    <ArrowLeft className="w-4 h-4" />
                    All Tsubaki Products
                  </Link>
                  <Link
