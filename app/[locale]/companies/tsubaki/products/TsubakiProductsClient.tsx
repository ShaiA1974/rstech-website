...
import { ExternalLink, Download, Filter, ChevronDown, ChevronUp, ArrowRight, Layers, Cable, Wrench } from "lucide-react";
import {
  TSUBAKI_PRODUCTS,
  TSUBAKI_CATEGORIES,
  TSUBAKI_INDUSTRIES,
  type TsubakiCategoryKey,
  type TsubakiIndustry,
} from "@/data/tsubaki-products";

const CATEGORY_STYLE: Record<string, { gradient: string; iconColor: string; icon: React.ElementType }> = {
  "cable-carriers": { gradient: "from-orange-100 to-orange-50", iconColor: "text-orange-400", icon: Layers },
  cables:           { gradient: "from-amber-100 to-amber-50",   iconColor: "text-amber-500",  icon: Cable  },
  systems:          { gradient: "from-slate-100 to-slate-50",   iconColor: "text-slate-400",  icon: Wrench },
};

export default function TsubakiProductsClient() {
  const searchParams = useSearchParams();
  const catParam = searchParams.get("cat") as TsubakiCategoryKey | null;
  const initialCat: TsubakiCategoryKey =
    catParam && TSUBAKI_CATEGORIES.some((c) => c.key === catParam) ? catParam : "all";

  const [activeCategory, setActiveCategory] = useState<TsubakiCategoryKey>(initialCat);
  const [activeIndustry, setActiveIndustry] = useState<TsubakiIndustry>("All");
  const [expandedSubs, setExpandedSubs] = useState<Record<string, boolean>>({});

  const filtered = useMemo(() => {
    return TSUBAKI_PRODUCTS.filter((p) => {
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-2xl p-5 mb-8 space-y-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {TSUBAKI_CATEGORIES.map(({ key, label }) => (
              <button
                key={key}
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors cursor-pointer ${
                  activeIndustry === ind
                    ...
                    : "bg-[#f0f4f8] text-[#334155] border border-[#e2e8f0] hover:border-orange-400/40 hover:text-orange-600"
                }`}
              >
                {ind}
              </button>
        ...
        <p className="text-xs text-[#7A8A99]">
          ...
          {activeIndustry !== "All" && (
            <span> for <span className="font-semibold text-orange-600">{activeIndustry}</span></span>
          )}
        </p>
      </div>
              ...
              {isExpanded(subcategory) && (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 p-6">
                  {products.map((product) => {
                    const style = CATEGORY_STYLE[product.category] ?? CATEGORY_STYLE["cable-carriers"];
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-5 flex flex-col gap-3 flex-1">
                          <h3 className="font-bold text-[#1a202c] text-sm leading-snug group-hover:text-orange-600 transition-colors">
                            {product.name}
                          </h3>
                          ...
                          <div className="mt-auto pt-3 border-t border-[#f0f4f8] flex items-center justify-between gap-2">
                            <Link
                              href={product.href ?? `/companies/tsubaki/products/${product.id}`}
                            ...
                            <div className="flex items-center gap-3">
                              <a
                                ...
                                className="inline-flex items-center gap-1 text-xs font-medium text-[#7A8A99] hover:text-orange-600 transition-colors cursor-pointer"
                              >
                                Supplier
